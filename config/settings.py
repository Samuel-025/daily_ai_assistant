...
class Settings:
    def __init__(self):
        ...
        # ── Default models (all verified active as of June 2026) ────────────
        self.default_models = {
            # Ollama  — any locally pulled model; llama3.2 is smallest/fastest
            "ollama":    os.getenv("OLLAMA_DEFAULT_MODEL",    "llama3.2"),

            # OpenAI  — gpt-4o is current flagship
            "openai":    os.getenv("OPENAI_DEFAULT_MODEL",    "gpt-4o"),

            # Anthropic — claude-3-5-sonnet still active
            "anthropic": os.getenv("ANTHROPIC_DEFAULT_MODEL", "claude-3-5-sonnet-20241022"),

            # Groq    — llama-3.1-70b-versatile was DECOMMISSIONED;
            #            replaced with llama-3.3-70b-versatile (June 2026)
            "groq":      os.getenv("GROQ_DEFAULT_MODEL",      "llama-3.3-70b-versatile"),

            # Cohere  — command-r-plus removed Sept 2025;
            #            command-a-03-2025 is current default (June 2026)
            "cohere":    os.getenv("COHERE_DEFAULT_MODEL",    "command-a-03-2025"),
        }
